...
def main(input_video_path: str,
    ...
    while True:
        ...
        res = depth_estimator.process_frame(frame)
        frame_to_write = cv2.resize(res, out_resolution)
        frame_to_write = cv2.applyColorMap(frame_to_write.astype(np.uint8), cv2.COLORMAP_INFERNO)
